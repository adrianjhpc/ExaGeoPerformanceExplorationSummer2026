...
# global indices, and the problem size on the normal build was pretty close
# to 25%
CLAMP_PROB_SIZE="${CLAMP_PROB_SIZE:-0}"
...
BYTES_PER_EQUATION="${BYTES_PER_EQUATION:-715}"
...
case "$IMPLEMENTATION" in
    ...
    intel_ilp64)
        ...
        if (( BYTES_PER_EQUATION == 715 )); then
            BYTES_PER_EQUATION=839
        fi
        ;;
    amd)
